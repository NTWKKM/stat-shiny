...
from utils.formatting import create_missing_data_report_html, format_ci_html, format_p_value
...
def prepare_data_for_ols(
    ...
    if constant_predictors:
        logger.warning(
            "Dropping constant predictor(s): %s", constant_predictors
        )
    ...
    missing_data_info = {
        ...
        'initial_n': initial_n,
        'summary_before': missing_summary.to_dict('records') if not missing_summary.empty else [],
        'constant_predictors_dropped': constant_predictors
    }
    
    logger.info(
        "OLS data prepared: %d/%d rows retained (%.1f%%), %d excluded",
        final_n, initial_n, 100 * final_n / initial_n if initial_n > 0 else 0, excluded_n
    )
...
def run_ols_regression(
    ...
    try:
        model = smf.ols(formula, data=df).fit(
            cov_type='HC3' if robust_se else cov_type
        )
...
def stepwise_selection(
    ...
    """
    logger.info("Starting stepwise selection: direction=%s, criterion=%s", direction, criterion)
    ...
    def fit_model(var_list: List[str]):
        ...
        if not var_list:
            ...
            predictors = " + ".join([f"Q('{v}')" if not v.isidentifier() else v for v in var_list])
    ...
    if current_model is None:
        return {'selected_vars': [], 'history': [], 'error': 'Initial model fitting failed'}
...
def format_stepwise_history(history: List[Dict]) -> pd.DataFrame:
    ...
    rows = []
    for step in history:
        action_emoji = {'initial': '🏁', 'add': '➕', 'remove': '➖'}.get(step['action'], '•')
        rows.append({
            'Step': step['iteration'],
            'Action': f"{action_emoji} {step['action'].title()}",
            'Variable': step.get('variable', '-'),
            'Criterion': f"{step['criterion']:.2f}" if np.isfinite(step['criterion']) else 'N/A',
...
def compare_models(
    ...
    for spec in model_specs:
        ...
        try:
            ...
            if predictors:
                pred_str = " + ".join([
                    f"Q('{v}')" if not v.isidentifier() else v 
                    for v in predictors
                ])
            ...
            results.append({
                ...
                'BIC': model.bic,
                'Log-Lik': model.llf,
                'F-stat': model.fvalue if np.isfinite(model.fvalue) else float('nan'),
                'F p-value': model.f_pvalue if np.isfinite(model.f_pvalue) else float('nan'),
    ...
    logger.info("Model comparison complete. Best AIC: %s", 
                comparison_df.loc[comparison_df['AIC'].idxmin(), 'Model'] if not comparison_df.empty else 'N/A')
...
def format_model_comparison(comparison_df: pd.DataFrame) -> str:
    ...
    df_display = comparison_df[[c for c in display_cols if c in comparison_df.columns]].copy()
    
    # Format numeric columns
    for col in ['R²', 'Adj R²']:
        if col in df_display.columns:
            df_display[col] = df_display[col].apply(lambda x: f"{x:.4f}" if np.isfinite(x) else 'N/A')
    for col in ['AIC', 'BIC', 'RSE']:
        if col in df_display.columns:
            df_display[col] = df_display[col].apply(lambda x: f"{x:.2f}" if np.isfinite(x) else 'N/A')
    ...
    return df_display.to_html(index=False, escape=False, classes='table table-striped table-hover', border=0)
...
def bootstrap_ols(
    ...
    """
    logger.info("Starting bootstrap OLS: n_bootstrap=%d, ci_level=%.2f", n_bootstrap, ci_level)
    ...
    if len(bootstrap_params) < n_bootstrap * 0.5:
        logger.warning("Too many failed bootstrap samples: %d/%d", failed_samples, n_bootstrap)
    ...
    logger.info("Bootstrap complete: %d successful samples, %d failed", 
                len(bootstrap_params), failed_samples)
...
def run_diagnostic_tests(results: OLSResult) -> List[DiagnosticResult]:
    ...
    try:
        ...
        diagnostics.append({
            ...
            'statistic': lm_stat,
            'p_value': lm_pvalue,
            'interpretation': 'Constant variance (homoscedasticity)' if lm_pvalue > 0.05 else 'Non-constant variance (heteroscedasticity)',
    ...
    try:
        ...
        diagnostics.append({
            ...
            'statistic': jb_stat,
            'p_value': jb_p,
            'interpretation': 'Normal distribution' if jb_p > 0.05 else 'Non-normal distribution',
            'passed': jb_p > 0.05
        })
    except Exception as e:
...
def create_residuals_vs_fitted_plot(
    ...
    try:
        ...
        rolling_mean = pd.Series(residuals.values[sorted_idx]).rolling(
    ...
    fig.update_layout(
        title=dict(
            text='<b>Residuals vs Fitted Values</b>',
            x=0.5,
            xanchor='center'
        ),
...
def create_scale_location_plot(
    ...
    try:
        sorted_idx = np.argsort(fitted_values.values)
        window = max(5, len(fitted_values) // 20)
        rolling_mean = pd.Series(sqrt_abs_resid[sorted_idx]).rolling(
            window=window, center=True, min_periods=1
        ).mean()
...
def create_residuals_vs_leverage_plot(
    ...
    colors = np.where(cooks_distance > cooks_threshold, 
    ...
    fig.update_layout(
        title=dict(
            text='<b>Residuals vs Leverage</b>',
            x=0.5,
            xanchor='center'
        ),
...
def format_ols_results(
    ...
    coef_df['P_fmt'] = coef_df['P-value'].apply(lambda x: format_p_value(x, use_style=True))
    ...
    def get_label(var_name: str) -> str:
        ...
        # Clean up Q() wrapper
        clean_name = var_name.replace("Q('", "").replace("')", "").replace("Intercept", "(Intercept)")
...
def generate_report(
    ...
    # --- Header Section ---
    reg_type_display = "Robust Regression (Huber M-estimator)" if regression_type == 'robust' else "Ordinary Least Squares (OLS)"
    html_parts.append(f"""
    ...
    if not vif_table.empty:
        ...
        high_vif = vif_table[vif_table['VIF'] > 5] if 'VIF' in vif_table.columns else pd.DataFrame()
        warning_html = ""
    ...
    if diagnostics:
        ...
        for d in diagnostics:
            ...
            p_fmt = format_p_value(d['p_value'], use_style=False) if np.isfinite(d['p_value']) else 'N/A'
            diag_rows.append(f"""
            <tr>
...
def analyze_linear_outcome(
    ...
    if predictor_cols is None:
        ...
        predictor_cols = [
            ...
            if c != outcome_name and c not in exclude_cols
        ]
    else:
    ...
    if regression_type == 'robust':
        results = run_robust_regression(
            df_clean, outcome_name, predictor_cols, 
            m_estimator=robust_estimator
        )
    else:
