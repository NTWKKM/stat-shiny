...
def apply_mcc(
    ...
    try:
        ...
        _, pvals_corrected, _, _ = smt.multipletests(
            p_vals_clean, alpha=alpha, method=method
        )
        ...
        return pd.Series(
            result, index=p_values.index if isinstance(p_values, pd.Series) else None
        )
...
def get_ci_configuration(
    ...
    if method_name == "auto":
        selected_method = determine_best_ci_method(
            n_samples, n_events, n_params, model_type
        )
        note = f"Auto-selected {selected_method.title()} based on sample size/events."

    return {"method": selected_method, "note": note}
