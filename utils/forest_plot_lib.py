...
class ForestPlot:
    ...
    def create(
        ...
        est_fmt = self.data[self.estimate_col].apply(
            lambda x: f"{x:.2f}" if np.isfinite(x) else "Inf"
        )
        low_fmt = self.data[self.ci_low_col].apply(
            lambda x: f"{x:.2f}" if np.isfinite(x) else "Inf"
        )
        high_fmt = self.data[self.ci_high_col].apply(
            lambda x: f"{x:.2f}" if np.isfinite(x) else "Inf"
        )
        ...
        if self.pval_col:
            self.data["__display_p"] = self._vectorized_format_pvalues(
                self.data[self.pval_col]
            )
        ...
        if show_sig_stars and self.pval_col:
            self.data["__sig_stars"] = self._vectorized_significance_stars(
                self.data[self.pval_col]
            )
        ...
        marker_colors, _ = (
            ...
            else ([color] * len(self.data), None)
        )

        has_pval = (
            self.pval_col is not None and not self.data[self.pval_col].isna().all()
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                error_x=dict(
                    ...
                    arrayminus=self.data[self.estimate_col]
        ...
        fig.update_layout(
            title=dict(
                text=title_with_summary, x=0.01, xanchor="left", font=dict(size=18)
            ),
            height=height,
            showlegend=False,
        ...
        for c in range(1, plot_col):
            fig.update_xaxes(
                visible=False, showgrid=False, zeroline=False, row=1, col=c
            )
            fig.update_yaxes(
                visible=False, showgrid=False, zeroline=False, row=1, col=c
            )
        ...
        fig.update_yaxes(
            visible=False, range=[-0.5, len(self.data) - 0.5], row=1, col=plot_col
        )
        fig.update_xaxes(
            title_text=x_label,
        ...
        headers = ["Variable", "Estimate (95% CI)"] + (
            ["P-value", f"{x_label} Plot"] if has_pval else [f"{x_label} Plot"]
        )
