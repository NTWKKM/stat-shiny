...
docs/ARCHITECTURE.md
 now explicitly documents the 30% numeric conversion threshold in
utils/data_cleaning.py
.
Documented the requirement for modules to call
prepare_data_for_analysis()
 to maintain data health and missingness reporting.
...
AI_CONTEXT.md
: Updated as a "Universal Entry Point" that summarizes these deep details for rapid LLM ingestion upon repository entry.
.cursorrules
: Enforces "Architecture-First" via Rule 0, ensuring bots visit these deep docs before coding.
Final Verification
Internal Consistency: Verified that the instructions in
AI_CONTEXT.md
 perfectly align with the detailed sections in
docs/ARCHITECTURE.md
.
...
-------------------
Walkthrough - Centralized Reporting & AI Bot Optimization
I have successfully centralized the report generation logic and optimized the repository for high-confidence AI bot collaboration.

🚀 Centralized Reporting Utility
I moved the fragmented
generate_report
 logic from individual modules into a unified utility: utils.formatting.generate_standard_report.

Key Improvements:
Universal Schema: Supports header, text,
table
,
plot
 (Plotly), interpretation, and
contingency_table
 elements.
...
Full Compatibility: Implemented wrappers in
correlation.py
,
diag_test.py
,
...
tvc_lib.py
.
🧠 Deep AI Bot Optimization

1. Architectural Standards
ARCHITECTURE.md
: Updated the Standardized Reporting Schema section. Documented the item-based report pattern and the 30% numeric conversion threshold.
AI_CONTEXT.md
: Points all bots directly to
generate_standard_report()
 as the source of truth for UI output.
2. Validation
...
correlation.py
 (Wrapper added)
diag_test.py
 (Wrapper added, 300+ lines of redundant CSS/HTML logic REMOVED)
survival_lib.py
 (Wrapper added)
tvc_lib.py
 (Wrapper added)
