...
from utils.tvc_lib import validate_long_format, transform_wide_to_long, fit_tvc_cox, check_tvc_assumptions
...
class TestTVCUtils:
    ...
    def test_validate_long_format_invalid_cols(self, long_data):
        is_valid, msg = validate_long_format(
            long_data, id_col='id', start_col='start', stop_col='stop', event_col='wrong_col'
        )
        assert not is_valid
