...
def test_e_value_rr_gt_1():
    ...
    assert pytest.approx(res["e_value_estimate"], 0.01) == 3.414
...
def test_e_value_ci():
    ...
    res = calculate_e_value(3.0, lower=2.0)
    assert pytest.approx(res["e_value_estimate"], 0.01) == 5.45
    assert pytest.approx(res["e_value_ci_limit"], 0.01) == 3.414


def test_e_value_ci_cross_null():
    """Test E-value when CI crosses 1."""
    # RR=1.5, Lower CI=0.8
    res = calculate_e_value(1.5, lower=0.8)
