...
def test_cleaned_data_improvement():
    ...
    # Verify improvements
    assert cleaned_df["numeric_with_issues"].dtype in [
        np.float64,
        float,
...
def test_table_one_workflow():
    ...
    print(
        f"Contains variable labels: {('age' in html_output.lower() or 'Age (years)' in html_output)}"
    )
    ...
    special_df = pd.DataFrame(
        {
            "special": [
                ...
                "50%",
                "100",
            ]  # Removed parentheses which cause issues
        }
    )
    # FIX: Renamed unused 'report' to '_report'
