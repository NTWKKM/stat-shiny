...
def test_diag_returns():
    ...
    res = diag_test.calculate_chi2(df_err, 'rater1', 'rater2', method="Fisher's Exact Test")
    assert len(res) == 5, f"calculate_chi2 (Fisher error) should return 5 values, got {len(res)}"
    assert res[2] == "Error: Fisher's Exact Test requires a 2x2 table.", f"Unexpected error message: {res[2]}"
    ...
    assert isinstance(res, pd.DataFrame), "calculate_descriptive should return a DataFrame"
