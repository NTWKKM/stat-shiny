...
class TestAdvancedFeatures:
    ...
    def causal_data(self):
        ...
        outcome_linear = 2 + 0.02 * age + 0.1 * severity - 0.5 * treated + np.random.normal(0, 0.5, n)
    ...
    def test_negative_binomial_flow(self, nb_data):
        ...
        est = irr_res["x"]["irr"]
        assert 1.2 < est < 2.2
        assert "Negative Binomial" in html_rep or "NB" in str(irr_res) # or implicit check
