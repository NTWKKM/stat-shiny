...
def select_variable_by_keyword(
    ...
    """
    Intelligently attempts to select a default variable from a sequence of column labels
    based on a sequence of keywords.

    Args:
        columns: Sequence of available column names or labels (can be any type convertible to string).
        keywords: Sequence of keywords to search for (case-insensitive).
        default_to_first: If True, returns the first column if no keyword match is found.

    Returns:
        The matched column label of original type T, or the first column (if default_to_first is True),
