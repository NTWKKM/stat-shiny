...
def data_server(  # noqa: C901, PLR0915, PLR0913
    ...
    """
    Initialize the server-side logic for the Data Management Shiny module.
    
    Parameters:
        df: Reactive storage for the active dataset.
        var_meta: Reactive storage for variable types, labels, mappings, and missing-value codes.
        uploaded_file_info: Reactive storage for uploaded-file metadata.
        df_matched: Reactive storage for a matched dataset.
        is_matched: Reactive flag indicating whether matching has been performed.
        matched_treatment_col: Reactive storage for the treatment column used in matching.
        matched_covariates: Reactive storage for covariates used in matching.
        mi_imputed_datasets: Reactive storage for datasets generated by multiple imputation.
    
    The function registers reactive handlers and renderers for data loading, metadata configuration, quality assessment, cleaning, imputation, transformations, matching state, and dataset previews.
    """
    ...
    def quality_warnings() -> list[str]:
        """Identify data-quality issues in the active dataset.
        
        Returns:
        	list[str]: A list of data-quality warnings, or an empty list when no dataset is loaded.
        """
        data = df.get()
        if data is None:
            return []
    ...
    def _infer_column_type(df_in: pd.DataFrame, col: str) -> tuple[str, list[dict]]:
        """
        Infer a column's data type and identify nonnumeric values in numeric-like columns.
        
        Parameters:
        	df_in (pd.DataFrame): Dataframe containing the column to classify.
        	col (str): Name of the column to inspect.
        
        Returns:
        	tuple[str, list[dict]]: The inferred type, either ``"Categorical"`` or ``"Continuous"``, and detected data-quality issues.
        """
        series = df_in[col]
    ...
    def _process_ingested_dataset(
        ...
        """
        Process an ingested dataset, initialize its metadata, and record detected data-quality issues.
        
        Parameters:
        	new_df (pd.DataFrame): The dataset to store and inspect.
        	file_name (str): The uploaded or generated dataset name.
        	preset_meta (dict[str, Any] | None): Existing metadata to preserve for known columns.
    ...
    def generate_example_data_logic():
        """Generate and load simulated clinical example data."""
    ...
    def _load_data_file(f: FileInfo):
        """
        Loads an uploaded data file into the active dataset, limiting it to 100,000 rows.
        """
        is_loading_data.set(True)
    ...
    def ui_preview_area():
        """
        Display the data preview area for the active dataset.
        
        Returns:
            The empty-state upload interface when no dataset is loaded, otherwise a
            namespaced dataframe preview output.
        """
        d = df.get()
        if d is None:
