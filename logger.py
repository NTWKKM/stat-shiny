...
class LoggerFactory:
    ...
    def configure(cls) -> None:
        ...
        try:
            ...
            log_format = DEFAULT_CONFIG.get('logging.format')
            date_format = DEFAULT_CONFIG.get('logging.date_format')
    ...
    def _setup_console_logging(cls, root_logger: logging.Logger, formatter: logging.Formatter) -> None:
        ...
        try:
            ...
            console_level = DEFAULT_CONFIG.get('logging.console_level', 'INFO')
    ...
    def _setup_file_logging(cls, root_logger: logging.Logger, formatter: logging.Formatter) -> None:
        """Setup file logging."""
        try:
            log_dir = Path(DEFAULT_CONFIG.get('logging.log_dir', 'logs'))
            log_dir.mkdir(exist_ok=True, parents=True)
            
            log_file = log_dir / DEFAULT_CONFIG.get('logging.log_file', 'app.log')
            max_size = DEFAULT_CONFIG.get('logging.max_log_size', 10485760)
            backup_count = DEFAULT_CONFIG.get('logging.backup_count', 5)
