...
def calculate_descriptive(df, col):
    """คำนวณสถิติพื้นฐาน (Shiny version - removed st.cache)"""
...
# ========================================
# 2. CI CALCULATION FUNCTIONS (คงเดิม)
# ========================================
...
def calculate_chi2(df, col1, col2, method='Pearson (Standard)', v1_pos=None, v2_pos=None):
    """2x2 contingency analysis (Shiny version - removed st.cache)"""
    ...
    try:
        ...
        if is_2x2:
            ...
            nnt_ci = calculate_ci_nnt(rd, rd_se)
            ...
            ppv, npv = a/(a+b) if (a+b)>0 else 0, d/(c+d) if (c+d)>0 else 0
            ppv_ci, npv_ci = calculate_ci_wilson_score(a, a+b), calculate_ci_wilson_score(d, c+d)
...
def analyze_roc(df, truth_col, score_col, method='delong', pos_label_user=None):
    ...
    if y_true_raw.nunique() != 2 or pos_label_user is None: return None, "Error: Needs binary class.", None, None
    ...
    stats_res = {"AUC": auc_val, "95% CI Lower": max(0, ci_lower), "95% CI Upper": min(1, ci_upper), "Sensitivity": tpr[best_idx], "Specificity": 1-fpr[best_idx], "Best Cut-off": thresholds[best_idx]}
...
# ========================================
# 7. REPORT GENERATION (Shiny version)
# ========================================

def generate_report(title, elements):
    """Generate HTML (Shiny version - Removed st calls)"""
    primary_color = COLORS['primary']
    text_color = COLORS['text']
    
    css = f"<style>body{{font-family:sans-serif; padding:20px; color:{text_color}}} h1{{color:{primary_color}; border-bottom:2px solid {primary_color}}} table{{width:100%; border-collapse:collapse; margin:20px 0}} th,td{{border:1px solid #ddd; padding:8px; text-align:left}} th{{background:{primary_color}; color:white}}</style>"
    ...
    for el in elements:
        ...
        if t == 'text': html += f"<p>{_html.escape(str(d))}</p>"
            ...
            if hasattr(d, 'to_html'): html += d.to_html(full_html=False, include_plotlyjs='cdn')
