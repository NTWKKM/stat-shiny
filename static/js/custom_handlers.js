$(document).ready(function () {
    // ==========================================
    // 1. DYNAMIC ELEMENT STYLE & TEXT HANDLERS
    // ==========================================
    ...
    // 1-Click Clipboard Copy Handler (for Word/Rich Text)
    ...
    function closeCommandPalette() {
        $('#cmd_palette_backdrop').removeClass('open');
    }
    ...
    $(document).on('keydown', function (e) {
        if ((e.metaKey || e.ctrlKey) && (e.key === 'k' || e.key === 'K')) {
            e.preventDefault();
            if ($('#cmd_palette_backdrop').hasClass('open')) {
                closeCommandPalette();
            } else {
                openCommandPalette();
            }
        } else if (e.key === 'Escape' && $('#cmd_palette_backdrop').hasClass('open')) {
            e.preventDefault();
            closeCommandPalette();
        }
    });
    ...
    // Accessible Plotly Chart Twin toggle
    $(document).on('click', '.chart-twin-toggle-btn', function () {
        var targetId = $(this).data('target-id');
        var view = $(this).data('view'); // 'chart' or 'table'
